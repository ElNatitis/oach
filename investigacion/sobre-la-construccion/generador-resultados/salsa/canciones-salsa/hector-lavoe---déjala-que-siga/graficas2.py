...
filename = "hector-lavoe---déjala-que-siga-(karaoplay.com).csv"
...
for instr, datos in df.groupby('instrumento'):
    ...
    # Graficando las tres variables en la misma gráfica
    ax.plot(datos.index, datos['volumen_norm'], label="Volumen", color='tab:orange', linewidth=2)
    ax.plot(datos.index, datos['dura_norm'], label="Duración", color='tab:green', linewidth=2)
    ax.plot(datos.index, datos['nota_norm'], label="Nota", color='tab:blue',linestyle=':',linewidth=2)
    
    ax.set_title(f"{nombre_instr} (número {instr})", fontsize=22)
    ax.set_ylabel("Valores Normalizados", fontsize=19)
    ax.set_xlabel("Semifusas", fontsize=19)
