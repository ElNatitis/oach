...
# oara las pruebas de la cancion con ceros
auxiliar_midi = []
...
prueba = rolitas[0]
prueba_prom = []
instrumentos = prueba.groupby('instrumento')# agrupamos para tener por separado cada instrumento de los eventos
log(f'{instrumentos}')
...
cancion_promediada = pandas.DataFrame(prueba_prom)
...
tempo = midi_pruebas.get_tempo_changes()[1][0] # para obtener una estimacion del tempo de la cancion
...
duracion_total = midi_pruebas.get_end_time() # para saber la duración total en segundos
...
for i, ins in instrumentos_con_ceros: # iteramos sobre los instrumentos
    ...
    if nota["termina"] < duracion_total:
        ...
        if espacios_llenados + espacios_silencio_final == espacios_necesarios:
            ...
            lof(f'vamos a ignorar {espacios_llenados + espacios_silencio_final - espacios_necesarios} espacios para evitar problemas')
