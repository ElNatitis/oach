...
void diag(struct matriz m, float *dp); // recibe una matriz y edita el arreglo 'dp' almacenando su diagonal principal 
void prn(struct matriz m, struct matriz inv); // imprime ambas matrices para monitoriar
void truco_uno(struct matriz m, struct matriz inv, float *dp); // se eliminan los elementos por debajo del pivote para que coincida con la matriz identidad por abajo
...
void truco_uno(struct matriz m, struct matriz inv, float *dp) // se eliminan los elementos por debajo del pivote para que coincida con la matriz identidad por abajo
{
  ...
  for(int i=0;i<m.columnas;i++)
  {
    ...
    for(int j=i+1;j<m.columnas;j++)
    {
      ...
      float c = -m.elemento[j*m.columnas+i];
...
struct matriz inversa(struct matriz m) // función principal declarada en 'matrices.h'
{
  ...
  //prn(m,inv);
  float *dp = calloc(m.filas,sizeof(float));diag(m,dp); // se declara el arreglo 'dp' para almacenar la diagonal principal
  truco_uno(m,inv,dp);
  truco_dos(m,inv);
  return inv;
}
