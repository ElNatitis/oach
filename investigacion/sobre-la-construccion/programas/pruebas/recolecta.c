  ...
  for(int i=0;i<n;i++)
  {
    ...
    if (getcwd(ruta, sizeof(ruta)) != NULL) // para obtener la ruta del directorio actual
    {
      ...
      if(fp)
      {
        ...
        char nombre_txt[100];
        snprintf(nombre_txt, sizeof(nombre_txt), "%s.txt", nombre_archivo);
        FILE *reporte = fopen(nombre_txt, "w");
        // para validar
        if (reporte == NULL)
        {
        ...
        char nombre_csv[100];
        snprintf(nombre_csv, sizeof(nombre_csv), "fluctuaciones-%s.csv", nombre_archivo);
        FILE *resultados = fopen(nombre_csv, "w");
        // para validar
        if (resultados == NULL)
        {
        ...
        fprintf(resultados, "instrumento,tamano_segmento,grado_polinomio,f_nota,f_volumen,f_dura\n");  // encabezado
        ...
        for(int w=0;w<canciones[i].numero_de_instrumentos;w++)
        {
          ...
          for(int u=0;u<9;u++)
          {
            ...
            for(int g=0;g<5;g++)
            {
              ...
              fprintf(resultados, "$%s,%d,%d,%d,%f,%f,%f\n",nombre_archivo, x.nombre,segmentos[u],grado[g],resultaos[1],resultaos[2],resultaos[3]);
      ...
      {
        printf("\nERROR, No se pudo obtener la ruta del directorio actual.\n");
        exit(1);
      }
    }
    else
